...
class FirebaseAiContentGenerator implements ContentGenerator {
  ...
  ({List<Tool>? generativeAiTools, Set<String> allowedFunctionNames}) _setupToolsAndFunctions({
    ...
    final DynamicAiTool<Map<String, Object?>>? finalOutputAiTool = isForcedToolCalling
    ...
    final List<AiTool<JsonMap>> allTools = isForcedToolCalling
        ...
        : availableTools;
    genUiLogger.fine('Available tools: ${allTools.map((t) => t.name).join(', ')}');
    ...
    for (final AiTool<JsonMap> tool in uniqueAiToolsByName.values) {
      ...
      if (tool.parameters != null) {
        final GeminiSchemaAdapterResult result = adapter.adapt(tool.parameters!);
      ...
      functionDeclarations.add(FunctionDeclaration(tool.name, tool.description, parameters: parameters ?? const {}));
      if (tool.name != tool.fullName) {
        functionDeclarations.add(
          FunctionDeclaration(tool.fullName, tool.description, parameters: parameters ?? const {}),
        );
      }
    }
    genUiLogger.fine(
    ...
    final allowedFunctionNames = <String>{...uniqueAiToolsByName.keys, ...toolFullNames};

    genUiLogger.fine('Allowed function names for model: ${allowedFunctionNames.join(', ')}');

    return (generativeAiTools: generativeAiTools, allowedFunctionNames: allowedFunctionNames);
  }
  ...
  Future<({List<FunctionResponse> functionResponseParts, Object? capturedResult})> _processFunctionCalls({
    ...
    genUiLogger.fine('Processing ${functionCalls.length} function calls from model.');
    final functionResponseParts = <FunctionResponse>[];
    for (final call in functionCalls) {
      genUiLogger.fine('Processing function call: ${call.name} with args: ${call.args}');
      if (isForcedToolCalling && call.name == outputToolName) {
        try {
          capturedResult = call.args['output'];
          genUiLogger.fine('Captured final output from tool "$outputToolName".');
        } catch (exception, stack) {
          genUiLogger.severe('Unable to read output: $call [${call.args}]', exception, stack);
        }
        genUiLogger.info(
          '****** Gen UI Output ******.\n'
      ...
      try {
        ...
        genUiLogger.severe('Error invoking tool ${aiTool.name} with args ${call.args}: ', exception, stack);
        toolResult = {'error': 'Tool ${aiTool.name} failed to execute: $exception'};
      }
    ...
    return (functionResponseParts: functionResponseParts, capturedResult: capturedResult);
  }

  Future<Object?> _generate({required Iterable<ChatMessage> messages, dsb.Schema? outputSchema}) async {
    ...
    final List<AiTool<JsonMap>> availableTools = [
      if (configuration.actions.allowCreate || configuration.actions.allowUpdate) ...[
        SurfaceUpdateTool(handleMessage: _a2uiMessageController.add, catalog: catalog, configuration: configuration),
        BeginRenderingTool(handleMessage: _a2uiMessageController.add),
      ],
      if (configuration.actions.allowDelete) DeleteSurfaceTool(handleMessage: _a2uiMessageController.add),
      ...additionalTools,
    ];

    // A local copy of the incoming messages which is updated with tool results
    // as they are generated.
    final List<Content> mutableContent = converter.toFirebaseAiContent(messages);

    final (:List<Tool>? generativeAiTools, :Set<String> allowedFunctionNames) = _setupToolsAndFunctions(
    ...
    final GeminiGenerativeModelInterface model = modelCreator(
      configuration: this,
      systemInstruction: systemInstruction == null ? null : Content.system(systemInstruction!),
      tools: generativeAiTools,
      toolConfig: isForcedToolCalling
          ? ToolConfig(functionCallingConfig: FunctionCallingConfig.any(allowedFunctionNames.toSet()))
...
With functions:
  ...
  ''');
      ...
      if (response.candidates.isEmpty) {
        genUiLogger.warning('Response has no candidates: ${response.promptFeedback}');
        return isForcedToolCalling ? null : '';
      }

      final Candidate candidate = response.candidates.first;
      final List<FunctionCall> functionCalls = candidate.content.parts.whereType<FunctionCall>().toList();
      ...
      genUiLogger.fine('Model response contained ${functionCalls.length} function calls.');
      mutableContent.add(candidate.content);
      genUiLogger.fine(
      ...
      final ({Object? capturedResult, List<FunctionResponse> functionResponseParts}) result =
      ...
      final List<FunctionResponse> functionResponseParts = result.functionResponseParts;
      ...
      if (!isForcedToolCalling && candidate.text != null && candidate.text!.trim().isNotEmpty) {
        genUiLogger.fine(
...
String _usageMetadata(UsageMetadata? metadata) {
  ...
  buffer.writeln('  toolUsePromptTokenCount: ${metadata.toolUsePromptTokenCount},');
  buffer.writeln('  promptTokensDetails: [');
  for (final ModalityTokenCount detail in metadata.promptTokensDetails ?? <ModalityTokenCount>[]) {
  ...
  for (final ModalityTokenCount detail in metadata.candidatesTokensDetails ?? <ModalityTokenCount>[]) {
    buffer.writeln('    ModalityTokenCount(');
  ...
  for (final ModalityTokenCount detail in metadata.toolUsePromptTokensDetails ?? <ModalityTokenCount>[]) {
    buffer.writeln('    ModalityTokenCount(');
    buffer.writeln('      ${detail.modality},');
    buffer.writeln('      ${detail.tokenCount},');
    buffer.writeln('    ),');
...
String _responseToString(GenerateContentResponse response) {
  ...
  for (final Candidate candidate in response.candidates) {
    ...
    for (final Part part in candidate.content.parts) {
      if (part is TextPart) {
        buffer.writeln('          TextPart(text: "${(part as TextPart).text}"),');
