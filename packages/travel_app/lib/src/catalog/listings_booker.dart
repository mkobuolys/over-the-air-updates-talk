...
final _schema = S.object(
  ...
  properties: {
    'listingSelectionIds': S.list(description: 'Listings to select among.', items: S.string()),
    'itineraryName': A2uiSchemas.stringReference(description: 'The name of the itinerary.'),
...
extension type _ListingsBookerData.fromMap(Map<String, Object?> _json) {
  ...
  List<String> get listingSelectionIds => (_json['listingSelectionIds']! as List).cast<String>();
...
final listingsBooker = CatalogItem(
  ...
  widgetBuilder: (context) {
    final listingsBookerData = _ListingsBookerData.fromMap(context.data as Map<String, Object?>);

    final ValueNotifier<String?> itineraryNameNotifier = context.dataContext.subscribeToString(
  ...
  exampleData: [
    () {
      ...
      final String listingSelectionId1 = BookingService.instance
          .listHotelsSync(HotelSearch(query: '', checkIn: start1, checkOut: end1, guests: 1))
          .listings
          .first
          .listingSelectionId;
      final String listingSelectionId2 = BookingService.instance
          .listHotelsSync(HotelSearch(query: '', checkIn: start2, checkOut: end2, guests: 1))
          .listings
          .last
          .listingSelectionId;
...
class _CustomRadio<T> extends StatefulWidget {
  ...
  const _CustomRadio({required this.value, required this.groupValue, required this.onChanged});
...
class _ListingsBookerState extends State<_ListingsBooker> {
  ...
  Widget build(BuildContext context) {
    final double grandTotal = _selections.fold<double>(0.0, (sum, listing) {
      final Duration duration = listing.search.checkOut.difference(listing.search.checkIn);
    ...
    return Column(
      children: [
        Padding(
          ...
          child: Text('Check out "${widget.itineraryName}"', style: Theme.of(context).textTheme.headlineMedium),
        ),
        ListView.builder(
          shrinkWrap: true,
                ...
                child: Column(
                  ...
                  children: [
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              Text(listing.location, style: Theme.of(context).textTheme.bodyMedium),
                            ],
                          ),
                        ),
                        Row(
                          children: [
                            ...
                            TextButton(
                              onPressed: () {
                                ...
                                final JsonMap resolvedContext = resolveContext(widget.dataContext, contextDefinition);
                                resolvedContext['listingSelectionId'] = listing.listingSelectionId;
                    ...
                    Row(
                      ...
                      children: [
                        Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text('Check-in', style: Theme.of(context).textTheme.bodySmall),
                            Text(dateFormat.format(checkIn), style: Theme.of(context).textTheme.bodyLarge),
                          ],
                        ),
                        Column(
                          crossAxisAlignment: CrossAxisAlignment.end,
                          children: [
                            Text('Check-out', style: Theme.of(context).textTheme.bodySmall),
                            Text(dateFormat.format(checkOut), style: Theme.of(context).textTheme.bodyLarge),
                          ],
                        ),
                      ],
                    ),
                    ...
                    Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        Text('Duration of stay:', style: Theme.of(context).textTheme.bodyMedium),
                        Text('${duration.inDays} nights', style: Theme.of(context).textTheme.bodyMedium),
                      ],
                    ),
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        Text('\$${totalPrice.toStringAsFixed(2)}', style: Theme.of(context).textTheme.titleMedium),
                      ],
                    ),
                  ],
                ),
            ...
            children: [
              Row(
                ...
                children: [
                  ...
                  Text('\$${grandTotal.toStringAsFixed(2)}', style: Theme.of(context).textTheme.headlineSmall),
                ],
              ),
              const Divider(),
              const SizedBox(height: spacing),
              ...
              const SizedBox(height: spacing * 2),
              _BookButton(bookingStatus: _bookingStatus, selectedCard: _selectedCard, onPressed: _book),
            ],
          ),
        ),
      ],
...
class _BookButton extends StatelessWidget {
  ...
  const _BookButton({required this.bookingStatus, required this.selectedCard, required this.onPressed});
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      ...
      child: _SubmitButton(
        onPressed: selectedCard != null && bookingStatus == BookingStatus.initial ? onPressed : null,
