...
class HotelSearchResult {
  ...
  static HotelSearchResult fromJson(JsonMap json) {
    return HotelSearchResult(
      listings: (json['listings']! as List).map((e) => HotelListing.fromJson(e as JsonMap)).toList(),
    );
  }
...
class HotelListing implements Listing {
  ...
  late final String description = '$name in $location, \$${pricePerNight.ceil()}';
  ...
  JsonMap toAiInput() {
    return {'description': description, 'images': images, 'listingSelectionId': listingSelectionId};
  }
}
...
class HotelSearch {
  ...
  HotelSearch({required this.query, required this.checkIn, required this.checkOut, required this.guests});
