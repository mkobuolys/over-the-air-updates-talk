...
extension type _DateTimeInputData.fromMap(JsonMap _json) {
  factory _DateTimeInputData({required JsonMap value, bool? enableDate, bool? enableTime, String? outputFormat}) =>
...
final dateTimeInput = CatalogItem(
  ...
  widgetBuilder: (itemContext) {
    final dateTimeInputData = _DateTimeInputData.fromMap(itemContext.data as JsonMap);
    final ValueNotifier<String?> valueNotifier = itemContext.dataContext.subscribeToString(dateTimeInputData.value);
        ...
        return ListTile(
          ...
          onTap: () async {
            ...
            if (dateTimeInputData.enableDate) {
              ...
              if (date != null) {
                itemContext.dataContext.update(DataPath(path), date.toIso8601String());
              }
            }
            if (dateTimeInputData.enableTime) {
              if (!itemContext.buildContext.mounted) return;
              ...
              if (time != null) {
                if (!itemContext.buildContext.mounted) return;
                itemContext.dataContext.update(DataPath(path), time.format(itemContext.buildContext));
              }
            }
          },
        );
