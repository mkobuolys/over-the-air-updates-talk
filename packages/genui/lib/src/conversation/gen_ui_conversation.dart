...
class GenUiConversation {
  ...
  GenUiConversation({
    ...
    _a2uiSubscription = contentGenerator.a2uiMessageStream.listen(genUiManager.handleMessage);
    _userEventSubscription = genUiManager.onSubmit.listen(sendRequest);
    _surfaceUpdateSubscription = genUiManager.surfaceUpdates.listen(_handleSurfaceUpdate);
    _textResponseSubscription = contentGenerator.textResponseStream.listen(_handleTextResponse);
  ...
  final ValueNotifier<List<ChatMessage>> _conversation = ValueNotifier<List<ChatMessage>>([]);
  ...
  void _handleSurfaceUpdate(GenUiUpdate update) {
    switch (update) {
      case SurfaceAdded():
        _conversation.value = [
          ..._conversation.value,
          AiUiMessage(definition: update.definition, surfaceId: update.surfaceId),
        ];
        onSurfaceAdded?.call(update);
      case SurfaceUpdated():
        final newConversation = List<ChatMessage>.from(_conversation.value);
        final int index = newConversation.lastIndexWhere((m) => m is AiUiMessage && m.surfaceId == update.surfaceId);
        final newMessage = AiUiMessage(definition: update.definition, surfaceId: update.surfaceId);
        if (index != -1) {
      ...
      case SurfaceRemoved():
        final newConversation = List<ChatMessage>.from(_conversation.value);
        newConversation.removeWhere((m) => m is AiUiMessage && m.surfaceId == update.surfaceId);
        _conversation.value = newConversation;
  ...
  void _handleError(ContentGeneratorError error) {
    ...
    final errorResponseMessage = AiTextMessage.text('An error occurred: ${error.error}');
